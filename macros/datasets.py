...
def get_models_dir():
   if 'CMSSW_BASE' not in os.environ:
      ...
      os.environ['CMSSW_BASE'] = '/afs/cern.ch/work/r/ratramon/Bparking/CMSSW_10_2_15/'
   
   mods = '/afs/cern.ch/work/r/ratramon/Bparking/CMSSW_10_2_15/src/LowPtElectrons/LowPtElectrons/macros/models/%s/' % ( tag)
...
def pre_process_data(dataset, features, for_seeding=False, keep_nonmatch=False):  
   ...
   features = list(set(features+#+['gen_pt', 'gen_eta', 
                                ...
                                # 'seed_trk_driven', 'seed_ecal_driven',
                                 ...
                                 ['BToKEE_pt', 'BToKEE_eta',
                                 'entry', 'weight','category']))
...
#     data = pd.concat((data, notmatched))
   ...
   mask = training_selection(data,0)
...
#   	   kmeans_model
#   	    )    
#  data['weight'] = weights*np.invert(data.signal) + data.signal
